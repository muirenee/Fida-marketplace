...
export const runtimeKeys=['PUBLIC_BASE_URL','CORS_ORIGIN','ADMIN_ALLOWED_IPS','MAINTENANCE_MODE','FEATURED_STORE_IDS'] as const;
...
export function validateRuntime(input:unknown,requestIp:string):RuntimeValues{
 ...
 for(const [key,value] of Object.entries(input)){
  ...
  if(['PUBLIC_BASE_URL'].includes(key)&&value){const u=new URL(value);if(u.protocol!=='https:'||u.username||u.password||u.search||u.hash)throw new Error('Access URLs must use HTTPS without credentials or query strings.');}
  if(key==='CORS_ORIGIN'&&value)for(const v of value.split(',')){const u=new URL(v.trim());if(u.protocol!=='https:'||u.origin!==v.trim())throw new Error('Use comma-separated HTTPS origins.');}
