...
export default function Home() {
  ...
  async function setTenantStatus(tenant: Tenant, status: string) {
    ...
    setError(null);
    try {
      await adminFetch(`tenants/${tenant.id}/status`, { method: 'PATCH', body: JSON.stringify({ status }) });
      await loadData();
    } catch (e) {
  ...
  async function saveMerchantSettings(tenant: Tenant, payload: MerchantSettingsPayload) {
    ...
    setError(null);
    try {
      await adminFetch(`tenants/${tenant.id}/settings`, { method: 'PATCH', body: JSON.stringify(payload) });
      setSelectedMerchant(null);
      await loadData();
    } catch (e) {
  ...
  async function setUserActive(target: AdminUser, isActive: boolean) {
    ...
    setError(null);
    try {
      await adminFetch(`users/${target.id}/active`, { method: 'PATCH', body: JSON.stringify({ isActive }) });
      await loadData();
    } catch (e) {
  ...
  async function approveDriver(target: AdminUser) {
    ...
    setError(null);
    try {
      await adminFetch('drivers', { method: 'POST', body: JSON.stringify({ userId: target.id }) });
      await loadData();
    } catch (e) {
  ...
  async function searchCatalog(event?: FormEvent) {
    ...
    try {
      ...
      const [productRows, categoryRows] = await Promise.all([
        adminFetch(`catalog/products${params.size ? `?${params.toString()}` : ''}`),
        adminFetch(`catalog/categories${categoryParams.size ? `?${categoryParams.toString()}` : ''}`),
      ]);
      setCatalogProducts(productRows as CatalogProduct[]);
      setCatalogCategories(categoryRows as CatalogCategory[]);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Unable to search catalog.');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function updateProduct(product: CatalogProduct, patch: { isActive?: boolean; isAvailable?: boolean }) {
    ...
    setError(null);
    try {
      await adminFetch(`catalog/products/${product.id}`, { method: 'PATCH', body: JSON.stringify(patch) });
      await searchCatalog();
    } catch (e) {
  ...
  async function updateCategory(category: CatalogCategory, isActive: boolean) {
    ...
    setError(null);
    try {
      await adminFetch(`catalog/categories/${category.id}/active`, { method: 'PATCH', body: JSON.stringify({ isActive }) });
      await searchCatalog();
    } catch (e) {
  ...
  return (
    <div className="admin-shell">
      ...
      <main className="content">
        ...
        {tab === 'users' && <UsersView users={users} query={userQuery} setQuery={setUserQuery} busy={busy} search={searchUsers} approveDriver={approveDriver} setActive={setUserActive} />}
...
function MerchantSettingsModal({ tenant, busy, onClose, onSave }: { tenant: Tenant; busy: boolean; onClose: () => void; onSave: (tenant: Tenant, payload: MerchantSettingsPayload) => Promise<void> }) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!Number.isFinite(minimum) || minimum < 0 || !Number.isFinite(delivery) || delivery < 0 || !Number.isFinite(service) || service < 0 || service > 100) { setLocalError('Enter valid non-negative fees. Service fee must be between 0 and 100%.'); return; }
    setLocalError(null);
    try { await onSave(tenant, { isAcceptingOrders: acceptingOrders, minimumOrder: minimum, defaultDeliveryFee: delivery, serviceFeePercent: service }); } catch { /* parent shows API error */ }
  }
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (!busy && event.currentTarget === event.target) onClose(); }}><form className="modal-card settings-modal" role="dialog" aria-modal="true" aria-label={`Settings for ${tenant.name}`} onSubmit={(e) => void submit(e)}><div className="modal-head"><div><div className="eyebrow">Merchant commercial settings</div><h2>{tenant.name}</h2><div className="modal-statuses"><span className={statusClass(tenant.status)}>{humanizeStatus(tenant.status)}</span><span className={statusClass(acceptingOrders ? 'active' : 'suspended')}>{acceptingOrders ? 'accepting orders' : 'orders paused'}</span></div></div><button className="btn" type="button" disabled={busy} onClick={onClose}>Close</button></div><div className="settings-body">{localError && <div className="error">{localError}</div>}<div className="settings-summary"><article className="detail-card"><div className="detail-label">Currency</div><div className="detail-value">{tenant.currency}</div></article><article className="detail-card"><div className="detail-label">Timezone</div><div className="detail-value">{tenant.timezone || 'Africa/Kigali'}</div></article><article className="detail-card"><div className="detail-label">Merchant type</div><div className="detail-value">{humanizeStatus(tenant.merchantType)}</div></article></div><label className="toggle-row"><div><strong>Accept new orders</strong><span>Turn this off to pause checkout for this merchant without suspending the account.</span></div><input type="checkbox" checked={acceptingOrders} onChange={(e) => setAcceptingOrders(e.target.checked)} /></label><div className="settings-grid"><div className="field"><label>Minimum order ({tenant.currency})</label><input className="input" type="number" min="0" step={tenant.currency === 'RWF' ? '1' : '0.01'} required value={minimumOrder} onChange={(e) => setMinimumOrder(e.target.value)} /><span className="field-help">Customer subtotal must reach this amount before checkout.</span></div><div className="field"><label>Default delivery fee ({tenant.currency})</label><input className="input" type="number" min="0" step={tenant.currency === 'RWF' ? '1' : '0.01'} required value={deliveryFee} onChange={(e) => setDeliveryFee(e.target.value)} /><span className="field-help">Added to new orders from this merchant.</span></div><div className="field"><label>Service fee (%)</label><input className="input" type="number" min="0" max="100" step="0.01" required value={serviceFeePercent} onChange={(e) => setServiceFeePercent(e.target.value)} /><span className="field-help">Calculated from the order subtotal.</span></div></div><div className="settings-note">Changes apply to new orders only. Existing order totals remain unchanged.</div></div><div className="settings-actions"><button className="btn" type="button" disabled={busy} onClick={onClose}>Cancel</button><button className="btn primary" disabled={busy}>{busy ? 'Saving…' : 'Save settings'}</button></div></form></div>;
}
